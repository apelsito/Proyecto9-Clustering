...
class AnalisisModelos:
    ...
    def calcular_metricas(self, modelo_nombre, metricas_logistica = False):
        """
        ...
        if metricas_logistica == True: 
            ...
            # Métricas para conjunto de entrenamiento
            metricas_train = {
                'r2_score': r2_score(self.y_test, pred_test),
                'MAE': mean_absolute_error(self.y_test, pred_test),
            ...
            # Métricas para conjunto de prueba
            metricas_test = {
                'r2_score': r2_score(self.y_train, pred_train),
                'MAE': mean_absolute_error(self.y_train, pred_train),
                'MSE': mean_squared_error(self.y_train, pred_train),
                'RMSE': np.sqrt(mean_squared_error(self.y_train, pred_train))
            }
            # Combinar métricas en un DataFrame
            return pd.DataFrame({"train": metricas_train, "test": metricas_test})
    ...
    def plot_shap_summary(self, modelo_nombre):
        ...
        if modelo_nombre in ["tree_regressor", "tree_classifier", "random_forest_regressor", "random_forest_classifier",
                              "gradient_boosting_regressor", "gradient_boosting_classifier", "xgboost_regressor", "xgboost_classifier"]:
